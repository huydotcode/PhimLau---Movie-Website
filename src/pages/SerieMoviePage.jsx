...
const SerieMoviePage = () => {
  ...
  const [filters, setFilters] = useState({
    ...
    year: [],
    lang: [],
    type: ["phim bộ"], // Mặc định loại phim là "phim bộ"
  ...
  const handleFilter = () => {
    ...
    // // Chuyển hướng sang SearchPage với các tiêu chí bộ lọc
    // navigate("/search", { state: { filters } });
  };

  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-2xl font-bold mb-4">Danh sách phim bộ</h1>
      {isLoading && <Loading isLoading />}
      {!isLoading && filteredResults.length === 0 && (
        <p className="text-gray-500">Không có phim bộ nào.</p>
      )}
